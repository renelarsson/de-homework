...
b) Create an external table
    ...
    1. Click on + CREATE DATASET in the left-hand menu if you don’t already have a dataset.
    2. Name the dataset (e.g., yellow_taxi) and click Create.
    3. Open the Query Editor and run the following SQL:
```sql
...
```
- Replace place holders with your names
- Click Run to create the external table.

c) Create a regular table

    1. Run the following SQL in the same Query Editor:

```sql
CREATE OR REPLACE TABLE `<PROJECT_ID>.yellow_taxi.yellow_tripdata`
AS
SELECT *
...
This query copies the data from the external table into a regular BigQuery table.
    ...
    2. Click Run to create the regular table.

d) Verify the tables

    1. Expand your dataset (yellow_taxi) in the menu.
...
1. Create and verify the dataset:
...
4. List the tables in your dataset:
