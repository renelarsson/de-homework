...
### Procedure to setup dbt Cloud CLI in Codespace (conflict-safe)
---

Ensure `dbt` is the *dbt Cloud CLI* (NOT dbt Core from a Python venv).

0) Make sure no Python venv is active (this prevents "wrong dbt" conflicts):
...
1) Install the dbt Cloud CLI (Linux):

**IMPORTANT**: use the official Linux install command from dbt docs/UI (it can change):
- https://docs.getdbt.com/docs/cloud/cloud-cli-installation  (Linux tab)
- After install, you should have a `dbt` binary on PATH:
```sh
tar -xf dbt_0.29.9_linux_amd64.tar.gz
./dbt --version
```
Expected help header includes: "The dbt CLI - an ELT tool for running SQL transformations..."

2) Download Cloud auth config (this is why your ~/.dbt/dbt_cloud.yml was missing)

- In dbt Cloud UI: main menu -> CLI -> "Download CLI configuration file"
- Then place it here:
```sh
mkdir -p ~/.dbt
```
- Move/rename the downloaded file into place (adjust filename if needed):
```sh
mv ~/Downloads/dbt_cloud.yml ~/.dbt/dbt_cloud.yml
ls -la ~/.dbt/dbt_cloud.yml
```

3) Ensure this repo is linked to your dbt Cloud project
- Already set in dbt_project.yml (project-id 70471823552275 is configured under `dbt-cloud:`)

4) Run commands (these execute in your dbt Cloud *development environment*)
```sh
...
**Answer:** If you run `dbt run --select int_trips_unioned`, dbt will build that model and its upstream dependencies, but not any downstream models, i.e. `stg_green_tripdata`, `stg_yellow_tripdata`, and `int_trips_unioned`.
