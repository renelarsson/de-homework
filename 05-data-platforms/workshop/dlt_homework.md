...
```sh
# Start a Python interactive shell in the terminal
uv run python
```
...
con = duckdb.connect('/workspaces/de-homework/05-data-platforms/workshop/taxi_pipeline.duckdb')

# Question 1: Start and End Date
print(con.execute("SELECT MIN(trip_pickup_date_time), MAX(trip_pickup_date_time) FROM taxi_data.rides").fetchall())

# Question 2: Proportion of Credit Card Payments
print(con.execute("SELECT COUNT(*) AS total, SUM(CASE WHEN payment_type = 'Credit' THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS credit_card_percentage FROM taxi_data.rides").fetchall())

# Question 3: Total Tips
print(con.execute("SELECT SUM(tip_amount) FROM rides").fetchall())
